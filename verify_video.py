...
def anlyze(verification_results):
    ...
    print(
        "date".rjust(11)
        + "observed_enter".rjust(20)
        + "displayed_enter".rjust(20)
        + "observed_leave".rjust(20)
        + "displayed_leave".rjust(20)
        + "fail_count".rjust(15)
    )
    for k, v in day_results.items():
        print(
            f"{k}".rjust(11)
            + f"{v.observed_enter}".rjust(20)
            + f"{v.displayed_enter}".rjust(20)
            + f"{v.observed_leave}".rjust(20)
            + f"{v.displayed_leave}".rjust(20)
            + f"{v.fail_count}".rjust(15)
        )
...
def extract_failed_clip(verify_result, out_dir_name="extracted_videos"):
    ...
    before_window = 7
    after_window = 3
    video_path = verify_result.path.parent.parent / "videos" / (verify_result.path.stem + ".mp4")
    ...
    for t in verify_result.times:
        out_path = out_dir / (verify_result.path.stem + f'_{t.strftime("%M_%S")}.mp4')
        start_frame = max(1, time2second(t.time()) * fps - before_window * fps)
        end_frame = min(frame_count, time2second(t.time()) * fps + after_window * fps)
        cap = cv2.VideoCapture(str(video_path))
