...
def extract(video_path, draw_mask=False, threshold=2000, mask_file_path=None, is_analyze=False, out_dir=None):
    """
    ...
    assert not draw_mask or mask_file_path is None, 'can not set mask_file_path if draw_mask is True'
    # parameters
    ...
    sections = [sect for sect in sections if (
        sect[1] - sect[0]) >= MIN_SECTION_LEN]
    ...
    writer = cv2.VideoWriter(os.path.join(
        out_dir, video_name), cv2.VideoWriter_fourcc(*'mp4v'), 5, (W, H))
    cap = cv2.VideoCapture(str(video_path))
    ret, frame = cap.read()
    counter = 1
    while ret:
